...
class GoodsLabel extends React.Component {
  ...
  handleSubmit = () => {
    this.goodsLabelBaseForm.validateFields((err, formData) => {
      if (err) {
        return;
      }
  ...
  addUpdateOne = async (formData) => {
    ...
    if (submitType) {
      formData.keys.forEach((item, index) => {
        arr.push({imgSrc: formData.imgSrc[item], name: formData.name[item]})
      })
      params.goodsKeywordsList = JSON.stringify(arr);
    } else {
      params.id = tableRowData.id;
      params.imgSrc = formData.imgSrc;
      params.name = formData.name
    }

    setCommon('loading', true);
    const { code, msg } = await GoodsCenterService.GoodsLabel[submitType ? 'addOne' : 'updateOne'](params);
  ...
  handleDelete = async (formData) => {
    const { code, msg } = await GoodsCenterService.GoodsLabel.updateOne({ id: formData.id, hidden: 0 });
    if (code !== SUCCESS_CODE) {
      helper.W(msg);
      return;
    }
  ...
  getData = async () => {
    ...
    const { code, msg, data } = await GoodsCenterService.GoodsLabel.selectAll({ ...searchData, pageNum, pageSize });
  ...
  renderImg = (text, record) => {
    return(
      <div>
        {record.imgSrc ? <img alt='展示图片' src={record.imgSrc} height="50"/> : null}
      </div>
    )
  };
      ...
      <div className="page-wrapper">
        ...
        <div className="page-wrapper-search">
          <SearchForm formList={columns()} showSearch={setAction(PATH, 'search')}
            ...
            {setAction(PATH, 'add') ? <Button type="primary" onClick={this.showModal.bind(this, true, null, null)}>添加标签</Button> : null}
          </SearchForm>
        </div>
        ...
        <Modal title={`${submitType ? '新增' : '编辑'}标签`}
          ...
          <GoodsLabelBaseForm ref={el => this.goodsLabelBaseForm = el} />
        </Modal>
      </div>
    )
  }
