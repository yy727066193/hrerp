...
import {setAction, getLoginInfo, getProvinceLinkage, searchList} from "../../../utils/public";
...
const PATH = 'InStoreOrderList';

const searchData = [
  {title: '退货入库单', dataIndex: 'likeSearch', formType: 'input',},
...
@inject('store')
@observer
class InStoreOrderList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  render() {
    ...
    const columns = () => {
      const arr = [];
      Columns.InStoreTableHead.forEach(item => {
  ...
  async getAllCompanyList() {
    ...
    const { data, code } = await BaseCenterService.Company.listAll({ companyIds });
  ...
  viewDetail(record) {
    this.props.history.push({pathname: 'OutAndInStoreOrder', state: {orderNumber: record.orderNumber}})
  }
...
export default InStoreOrderList;
