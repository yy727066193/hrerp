...
class BaseForm extends React.Component {
  ...
  setFormData = () => {
    ...
    if(!this.props.setMarketRowData) {
      return;
    }
    const tableRowData = this.props.setMarketRowData.goodsExtendedAttribute 
                          ? JSON.parse(JSON.stringify(this.props.setMarketRowData.goodsExtendedAttribute)) 
                          : undefined;
    const { mainImgsFileList, videosFileList, detailImgsFileList } = this.state;
    const tableRowDataPresentation = this.props.setMarketRowData.goodsPresentation;

    if(!tableRowData) {
      return;
    }
    const goodsKeywordsDataArr = [];
    if(tableRowData.goodsKeywordsListData.length) {
      tableRowData.goodsKeywordsListData.forEach(item => {
