...
class PopoverTableUp extends React.Component {
  ...
  changeStartStop = async (text, record, index) => {
    ...
    const params = {hrNo: record.hrNo, list: [{companyId: record.companyId, parentPutOn: record.parentPutOn ? 0 : 1, name: record.name}], goodsGenre: 2}
    const { data, code, msg } = await GoodsCenterService.MealList.updateStatus(params);
    if (code !== SUCCESS_CODE) {
      tableDataList[index].parentPutOn = record.parentPutOn;   // 上下架没有成功不更改状态
      this.setState({tableDataList});
      helper.W(msg);
      return;
    }
    helper.S();
    tableDataList[index].parentPutOn = data.list[0].parentPutOn;
    this.setState({tableDataList});
  };
