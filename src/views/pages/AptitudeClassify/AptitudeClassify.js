...
import AptitudeClassifyBaseForm from './AptitudeClassifyBaseFrom';
import AptitudeCenterService from "../../../service/AptitudeCenterService";
import {SUCCESS_CODE} from "../../../conf";
import helper from "../../../utils/helper";

const PATH = 'aptitudeClassify';

@inject('store')
@observer
class AptitudeClassify extends React.Component {

  getData = async () => {   // 初始化数据
    const { searchData, pageNum, pageSize, setCommon } = this.props.store;
    setCommon('tableLoading', true);
    const { code, msg, data } = await AptitudeCenterService.AptitudeClassify.listAll({ ...searchData, pageNum, pageSize });
  ...
  handleSubmit = () => {   // 提交新增或修改数据
    this.aptitudeClassifyBaseForm.validateFields((err, formData) => {
      if (err) {
        return;
      }
  ...
  addUpdateOne = async (formData) => {   // 新增或修改
    ...
    if (submitType) {   // 新增
      ...
      params.typeDesc = formData.typeDesc;
    ...
    const { code, msg } = await AptitudeCenterService.AptitudeClassify[submitType ? 'addOne' : 'updateOne'](params);
  ...
  handleDelete = async (formData) => {   // 删除
    const { code, msg } = await AptitudeCenterService.AptitudeClassify.delete({ id: formData.id });
    if (code !== SUCCESS_CODE) {
      helper.W(msg);
      return;
    }
      ...
      <div className="page-wrapper">
        ...
        <div className="page-wrapper-search">
          <SearchForm formList={columns()} showSearch={setAction(PATH, 'search')}
            ...
            {setAction(PATH, 'add') ? <Button type="primary" onClick={this.showModal.bind(this, true, null, null)}>添加分类</Button> : null}
          </SearchForm>
        </div>
        ...
        <Modal title={`${submitType ? '新增' : '编辑'}分类`}
          ...
          <AptitudeClassifyBaseForm ref={el => this.aptitudeClassifyBaseForm = el} />
        </Modal>
      </div>
    )
  }
...
export default AptitudeClassify;
